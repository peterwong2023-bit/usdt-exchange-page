...
class RiskMonitoringSystemBase {
    ...
    openAuditDrawer(id, type = 'user') {
        ...
        if (overlay && drawer && iframe) {
            ...
            if (type === 'user') {
                iframe.src = `user-detail-v6.html?user_id=${id}&_t=${cacheBuster}`;
                if (titleEl) titleEl.innerHTML = '<i class="fas fa-user-shield"></i> 风险用户审核详情';
            } else if (type === 'syndicate') {
                iframe.src = `group-graph.html?group_id=${id}&_t=${cacheBuster}`;
                if (titleEl) titleEl.innerHTML = '<i class="fas fa-project-diagram"></i> 团伙关系穿透分析';
            } else if (type === 'event') {
                iframe.src = `event-detail-readonly.html?event_id=${id}&_t=${cacheBuster}`;
            ...
            container.innerHTML = filtered.map(rule => `
                <div class="rcm-rule-row">
                    <div class="rcm-rule-info">
                        <div style="width: 36px; height: 36px; border-radius: 8px; background: ${this.getCategoryColor(rule.type).bg}; color: ${this.getCategoryColor(rule.type).fg}; display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0;">
                            <i class="fas ${this.getCategoryIcon(rule.type)}"></i>
                        </div>
                        <div style="min-width: 0;">
                            <div style="font-size: 14px; font-weight: 700; color: #0f172a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${rule.name}</div>
                            <div style="font-size: 11px; color: #94a3b8; font-family: 'JetBrains Mono';">ID: ${rule.id}</div>
                        </div>
                        <div class="rcm-category-badge ${rule.type}">${this.getCategoryLabel(rule.type)}</div>
                        <div class="rcm-threshold-summary">${rule.summary}</div>
                    </div>
